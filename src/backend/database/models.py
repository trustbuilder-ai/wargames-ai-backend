...
class Users(SQLModel, table=True):
    ...
    user_chat_template_context: list["UserChatTemplateContext"] = Relationship(
        back_populates="user"
    )
...
class UserChatTemplateContext(SQLModel, table=True):
    ...
    last_message_version: int = Field(
        sa_column=Column(
            "last_message_version", BigInteger, server_default=text("'0'::bigint")
        )
    )
    processing_token: str | None = Field(
        default=None, sa_column=Column("processing_token", String)
    )
    processing_started_at: datetime | None = Field(
        default=None, sa_column=Column("processing_started_at", DateTime(True))
    )
    last_message_id: int | None = Field(
        default=None, sa_column=Column("last_message_id", BigInteger)
    )

    chat_template: Optional["ChatTemplate"] = Relationship(
        back_populates="user_chat_template_context"
    )
    ...
    user_chat_template_context_messages: list["UserChatTemplateContextMessages"] = (
        Relationship(back_populates="user_chat_template_context")
    )
...
class ChallengeEvaluations(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["user_chat_template_context.id"],
            ondelete="CASCADE",
            name="challenge_evaluation_user_chat_template_context_id_fkey",
        ),
    ...
    user_chat_template_context_id: int | None = Field(
        default=None, sa_column=Column("user_chat_template_context_id", Integer)
    )
    ...
    user_chat_template_context: Optional["UserChatTemplateContext"] = Relationship(
...
class UserChatTemplateContextMessages(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["user_chat_template_context.id"],
            ondelete="CASCADE",
            name="user_chat_template_context_messages_user_chat_template_context_",
        ),
        PrimaryKeyConstraint("id", name="user_message_metadata_pkey"),
        Index(
            "idx_user_chat_template_context_messages_context_id",
            "user_chat_template_context_id",
        ),
    ...
    user_chat_template_context_id: int = Field(
        sa_column=Column("user_chat_template_context_id", Integer)
    )
    content: str = Field(sa_column=Column("content", Text))
    content_type: str = Field(sa_column=Column("content_type", String))
    model: str | None = Field(default=None, sa_column=Column("model", Text))
    is_user_provided: bool | None = Field(
        default=None, sa_column=Column("is_user_provided", Boolean)
    )
    role: str | None = Field(default=None, sa_column=Column("role", String))
    parent_message_id: int | None = Field(
        default=None, sa_column=Column("parent_message_id", Integer)
    )

    user_chat_template_context: Optional["UserChatTemplateContext"] = Relationship(
        back_populates="user_chat_template_context_messages"
    )
